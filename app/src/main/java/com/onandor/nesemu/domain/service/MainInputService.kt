...
import com.onandor.nesemu.domain.service.InputService.Event
import com.onandor.nesemu.domain.service.InputService.State
...
@Singleton
class MainInputService @Inject constructor(
    ...
    private var player1Device: NesInputDevice? = null
    private var player2Device: NesInputDevice? = null
        ...
        override fun onInputDeviceRemoved(deviceId: Int) {
            ...
            nesDevice?.let {
                if (player1Device == it) {
                    ...
                    _events.tryEmit(Event.OnInputDeviceDisconnected(InputService.PLAYER_1))
                } else if (player2Device == it) {
                    player2Device = player2Device?.copy(id = null)
                    _events.tryEmit(Event.OnInputDeviceDisconnected(InputService.PLAYER_2))
                }
                updateState()
                persistInputDevices()
            }
    ...
    override fun changeInputDevice(playerId: Int, device: NesInputDevice?) {
        if (device == null) {
            if (playerId == InputService.PLAYER_1) {
                player1Device = null
            } else {
                player2Device = null
            }
    ...
    override fun onInputEvents(deviceId: Int, buttonStates: Map<NesButton, NesButtonState>) {
    ...
    override fun onInputEvent(deviceId: Int, button: NesButton, state: NesButtonState) {
        if (player1Device?.id == deviceId) {
    ...
    override fun onInputEvent(event: KeyEvent): Boolean {
        ...
        val playerId = if (player1Device?.id == device.id) {
            InputService.PLAYER_1
        } else if (player2Device?.id == device.id) {
            InputService.PLAYER_2
        } else {
    ...
    override fun getButtonStates(playerId: Int): Int {
        val buttonStateMap = if (playerId == InputService.PLAYER_1) player1Buttons else player2Buttons
        var buttonStates = 0
    ...
    override fun changeButtonMapping(
        keyCode: Int,
        button: NesButton,
        playerId: Int,
